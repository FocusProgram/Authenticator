  ...
  for (const hash in backupData) {
    ...
    if (unknownStorageItem.dataType === "EncOTPStorage") {
      ...
      try {
        ...
        // Clean up: only keep the actual entry data, remove EncOTPStorage-specific fields
        storageItem = {
          ...decrypted,
          hash: hash,
          index: unknownStorageItem.index,
        };
        // Explicitly remove EncOTPStorage fields and set encrypted to false
        ...
        // Force encrypted to false after deletion to ensure it's not in encrypted state
